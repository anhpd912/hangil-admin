...
import { LessonContentEditor, contentToMarkdown, markdownToContent } from "./lesson-content-editor";
...
export function LessonForm({ lesson, onSaved }: LessonFormProps) {
  ...
  const [markdown, setMarkdown] = useState(contentToMarkdown(lesson?.content));
  ...
  return (
    <div className="rounded-2xl border border-dark/15 bg-cream p-6">
      ...
      <form id="lesson-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <LessonContentEditor value={markdown} onChange={setMarkdown} />
